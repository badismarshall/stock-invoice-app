...
const FR = {
  sort: "Trier",
  sortBy: "Trier par",
  noSortApplied: "Aucun tri appliqué",
  modifySort: "Modifiez le tri pour organiser les lignes.",
  addSortToOrganize: "Ajoutez un tri pour organiser les lignes.",
  addSort: "Ajouter un tri",
  resetSort: "Réinitialiser le tri",
  noFieldsFound: "Aucun champ trouvé.",
  searchFields: "Rechercher des champs...",
  // For select orders, override label with French below
  asc: "Ordre croissant",
...
export function DataTableSortList<TData>({
  ...
  const { columnLabels, columns } = React.useMemo(() => {
    ...
    for (const column of table.getAllColumns()) {
      ...
      // If label is not already translated, you should translate here if needed
        ...
        >
          <div className="flex flex-col gap-1">
            <h4 id={labelId} className="font-medium leading-none">
              {sorting.length > 0 ? FR.sortBy : FR.noSortApplied}
            </h4>
            <p
              id={descriptionId}
              className={cn(
...
function DataTableSortItem({
  ...
  // Replace default sort orders labels by French equivalents
  function getFrenchSortOrderLabel(value: string) {
    if (value === "asc") return FR.asc;
    if (value === "desc") return FR.desc;
    return value;
  }
    ...
    <SortableItem value={sort.id} asChild>
      ...
      >
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          ...
          >
            <Command>
              <CommandInput placeholder={FR.searchFields} />
              <CommandList>
                <CommandEmpty>{FR.noFieldsFound}</CommandEmpty>
                <CommandGroup>
                  {columns.map((column) => (
                    <CommandItem
                      key={column.id}
          ...
          >
            {dataTableConfig.sortOrders.map((order) => (
              <SelectItem key={order.value} value={order.value}>
                {getFrenchSortOrderLabel(order.value)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
