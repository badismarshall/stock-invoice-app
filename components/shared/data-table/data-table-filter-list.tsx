...
// Imports remain en anglais, par convention projet
...
import { getDefaultFilterOperator, getFilterOperators } from "@/lib/data-table/data-table";
...
export function DataTableFilterList<TData>({
  ...
  const [filters, setFilters] = useQueryState(
    table.options.meta?.queryKeys?.filters ?? "filtres",
        ...
        >
          <div className="flex flex-col gap-1">
            <h4 id={labelId} className="font-medium leading-none">
              {filters.length > 0 ? "Filtres" : "Aucun filtre appliqué"}
            </h4>
            <p
              id={descriptionId}
              className={cn(
            ...
            >
              {filters.length > 0
                ? "Modifiez les filtres pour affiner vos lignes."
                : "Ajoutez des filtres pour affiner vos lignes."}
            </p>
          </div>
          {filters.length > 0 ? (
    ...
    <SortableItem value={filter.filterId} asChild>
      ...
      >
        <div className="min-w-[72px] text-center">
          {index === 0 ? (
            <span className="text-muted-foreground text-sm">Où</span>
          ) : index === 1 ? (
            <Select
              value={joinOperator}
            ...
            >
              <SelectTrigger
                aria-label="Sélectionnez l'opérateur de jonction"
                aria-controls={joinOperatorListboxId}
                className="h-8 rounded lowercase data-size:h-8"
              >
                <SelectValue placeholder={joinOperator === "and" ? "Et" : joinOperator === "or" ? "Ou" : joinOperator} />
              </SelectTrigger>
              <SelectContent
              ...
              >
                {dataTableConfig.joinOperators.map((joinOperator) => (
                  <SelectItem key={joinOperator} value={joinOperator}>
                    {joinOperator === "and" ? "Et" : joinOperator === "or" ? "Ou" : joinOperator}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
            ...
            >
              <span className="truncate">
                {columns.find((column) => column.id === filter.id)?.columnDef
                  .meta?.label ?? "Sélectionner un champ"}
              </span>
          ...
          >
            <Command>
              <CommandInput placeholder="Rechercher des champs..." />
              <CommandList>
                ...
                placeholder={
                  (() => {
                    switch (filter.operator) {
                      ...
                      case "inArray": return "dans";
                      case "notInArray": return "pas dans";
            ...
            {filterOperators.map((operator) => (
              ...
              >
                {operator.label === 'contains'
                  ...
                  ? 'égal à'
                  : operator.label === 'notEquals'
                  ? 'différent de'
                  : operator.label === 'isEmpty'
                  ? 'est vide'
                  : operator.label === 'isNotEmpty'
                  ? "n'est pas vide"
                  : operator.label === 'greaterThan'
                  ? 'supérieur à'
                  : operator.label === 'lessThan'
                  ? 'inférieur à'
                  : operator.label === 'isBetween'
                  ? 'est entre'
                  : operator.label === 'in'
                  ? 'dans'
                  : operator.label === 'notIn'
                  ? 'pas dans'
                  : operator.label === 'startsWith'
                  ? 'commence par'
                  : operator.label === 'endsWith'
                  ? 'se termine par'
                  : operator.label}
              </SelectItem>
            ))}
        ...
        <SortableItemHandle asChild>
          <Button variant="outline" size="icon" className="size-8 rounded" aria-label="Déplacer le filtre">
            <GripVertical />
          </Button>
        </SortableItemHandle>
      </li>
  ...
  if (filter.operator === "isEmpty" || filter.operator === "isNotEmpty") {
    return (
      <div
        id={inputId}
        role="status"
        aria-label={`${columnMeta?.label} filtre ${filter.operator === "isEmpty" ? "est vide" : "n'est pas vide"}`}
        aria-live="polite"
    ...
    case "range": {
      ...
      return (
        <Input
          ...
          aria-label={`Valeur du filtre ${columnMeta?.label}`}
          aria-describedby={`${inputId}-description`}
          inputMode={isNumber ? "numeric" : undefined}
          placeholder={columnMeta?.placeholder ?? "Entrer une valeur..."}
      ...
      return (
        ...
        >
          <SelectTrigger
            ...
            aria-label={`Filtre booléen ${columnMeta?.label}`}
            className="h-8 w-full rounded data-size:h-8"
          >
            <SelectValue placeholder={filter.value === "true" ? "Vrai" : "Faux"} />
          </SelectTrigger>
        ...
        >
          <FacetedTrigger asChild>
            <Button
              id={inputId}
              aria-controls={inputListboxId}
              aria-label={`Valeur${multiple ? "s" : ""} de filtre ${columnMeta?.label}`}
              variant="outline"
              size="sm"
            ...
            >
              <FacetedBadgeList
                ...
                placeholder={
                  ...
                  `Sélectionner une option${multiple ? "s" : ""}...`
                }
              />
            </Button>
          </FacetedTrigger>
          <FacetedContent id={inputListboxId} className="w-[200px]">
            <FacetedInput
              aria-label={`Rechercher les options ${columnMeta?.label}`}
              placeholder={columnMeta?.placeholder ?? "Rechercher des options..."}
            />
            <FacetedList>
              <FacetedEmpty>Aucune option trouvée.</FacetedEmpty>
              <FacetedGroup>
        ...
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger asChild>
            <Button
              id={inputId}
              aria-controls={inputListboxId}
              aria-label={`Filtre de date ${columnMeta?.label}`}
              variant="outline"
              size="sm"
              className={cn(
          ...
          >
            {filter.operator === "isBetween" ? (
              <Calendar
                aria-label={`Sélectionner une plage de dates pour ${columnMeta?.label}`}
                autoFocus
              ...
              <Calendar
                aria-label={`Sélectionner une date pour ${columnMeta?.label}`}
                autoFocus
                captionLayout="dropdown"
                mode="single"
                selected={
