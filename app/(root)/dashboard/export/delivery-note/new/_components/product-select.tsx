...
interface ProductSelectProps {
  ...
  disabled?: boolean;
  placeholder?: string;
  initialProductData?: { id: string; name: string; code: string } | null;
}
...
export function ProductSelect({
  ...
  const [selectedProductData, setSelectedProductData] = React.useState<ProductData | null>(
    initialProductData ? {
  ...
  // Update selectedProductData when initialProductData changes
  React.useEffect(() => {
    if (initialProductData && value === initialProductData.id) {
  ...
  // Load selected product on mount if value is set but product is not in list
  React.useEffect(() => {
    if (value && !selectedProductData) {
      const loadSelectedProduct = async () => {
        try {
          ...
          // If not in list, search for it by fetching products with search
          // We'll search by ID or try to find it in paginated results
          const result = await getActiveProductsPaginated({
            page: 1,
            limit: 1000, // Large limit to find the product
          });

          if (result.data) {
            const foundProduct = result.data.find((p) => p.id === value);
            if (foundProduct) {
  ...
  }, [value, selectedProductData, products]);
