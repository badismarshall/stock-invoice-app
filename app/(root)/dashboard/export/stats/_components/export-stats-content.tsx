...
import { getExportSalesStats, type ExportSalesStats } from "../../_lib/stats-actions";
...
export function ExportStatsContent() {
  const [stats, setStats] = React.useState<ExportSalesStats | null>(null);
  ...
  React.useEffect(() => {
    const fetchStats = async () => {
      ...
      setError(null);
      try {
        const result = await getExportSalesStats({
          startDate,
          endDate,
        });
        if (result.error) {
  ...
  if (error) {
    return (
      <div className="h-full flex-1 flex-col space-y-8 p-8">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold tracking-tight">État de Vente Export</h2>
          <p className="text-muted-foreground">
            Statistiques détaillées des ventes export
          </p>
        </div>
    ...
    <div className="h-full flex-1 flex-col space-y-8 p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {stats.byCurrency.length > 0 && (
          <CurrencyBreakdown data={stats.byCurrency} />
        )}
      </div>
