...
export function CreateCancellationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigate and refresh - use export path
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Nouvelle Annulation Export</h1>
            <p className="text-muted-foreground">
              Créer une annulation partielle de bon de livraison d'export
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card rounded-xl shadow-sm border border-border p-6 space-y-6">
        ...
        {formData.clientId && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">Produits vendus au client (Export)</h3>
