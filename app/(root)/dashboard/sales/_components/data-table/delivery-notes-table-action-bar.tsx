...
import { Download, Trash2, FileText, FileDown, ChevronDown } from "lucide-react";
...
export function DeliveryNotesTableActionBar({ table }: DeliveryNotesTableActionBarProps) {
  ...
  const exportSelectedToPDF = React.useCallback(async () => {
    ...
    try {
      ...
      // Get full delivery notes with items for selected IDs
      ...
      // Generate PDF content (same logic as export-delivery-notes-buttons)
      ...
      // Create HTML content for PDF (same as export-delivery-notes-buttons)
      const htmlContent = `
        <!DOCTYPE html>
        <html>
          <head>
  ...
  const exportSelectedToXLSX = React.useCallback(async () => {
    ...
    try {
      ...
      // Flatten all items (same logic as export-delivery-notes-buttons)
      const allItems = deliveryNotes.flatMap((note) =>
        note.items.map((item) => ({
