...
export interface LocalSalesStats {
  summary: {
...
/**
 * Get comprehensive statistics for local sales
 */
export async function getLocalSalesStats(input?: {
  startDate?: Date;
  endDate?: Date;
}): Promise<{ data: LocalSalesStats | null; error: string | null }> {
  try {
    ...
    const deliveryNotesStats = await db
      ...
      .where(
        and(
          eq(deliveryNote.noteType, "local"),
    ...
    console.error("Error getting local sales stats:", err);
    return {
      data: null,
