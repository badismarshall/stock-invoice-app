...
export function PartnersTableActionBar({ table, type }: PartnersTableActionBarProps) {
  ...
  const exportSelectedToPDF = React.useCallback(async () => {
    ...
    try {
      ...
      // Get full partners for selected IDs
      const { getPartnersByIds } = await import("../../_lib/export-actions");
      ...
      const [partnersResult, companyResult] = await Promise.all([
        getPartnersByIds({ ids: selectedIds, type }),
        getCompanySettings(),
      ]);
  ...
  const exportSelectedToXLSX = React.useCallback(async () => {
    ...
    try {
      ...
      const partnersResult = await getPartnersByIds({ ids: selectedIds, type });
      ...
      <div className="flex items-center gap-1.5">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <DataTableActionBarAction
              size="icon"
              tooltip={type === "fournisseur" ? "Exporter les fournisseurs sélectionnés" : "Exporter les clients sélectionnés"}
        ...
        <DataTableActionBarAction
          size="icon"
          tooltip={type === "fournisseur" ? "Supprimer les fournisseurs" : "Supprimer les clients"}
