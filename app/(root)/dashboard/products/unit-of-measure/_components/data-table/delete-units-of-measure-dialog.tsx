...
const fr = {
  ...
  success: "Unités de mesure supprimées",
  error: "Erreur lors de la suppression",
};

interface DeleteUnitsOfMeasureDialogProps
  extends React.ComponentPropsWithoutRef<typeof Dialog> {
  unitsOfMeasure: Row<UnitOfMeasureDTOItem>["original"][];
  showTrigger?: boolean;
  onSuccess?: () => void;
}

export function DeleteUnitsOfMeasureDialog({
  unitsOfMeasure,
  showTrigger = true,
  onSuccess,
  ...props
}: DeleteUnitsOfMeasureDialogProps) {
  ...
  function onDelete() {
    startDeleteTransition(async () => {
      const { error } = await deleteUnitsOfMeasure({
        ids: unitsOfMeasure.map((unit) => unit.id),
      });
  ...
  const unitCount = unitsOfMeasure.length;
  const unitLabel =
    unitCount === 1
      ? `1 ${fr.unit}`
      : `${unitCount} ${fr.units}`;
        ...
        {showTrigger ? (
          <DialogTrigger asChild>
            <Button variant="outline" size="sm">
              ...
              {fr.deleteMany(unitsOfMeasure.length)}
            </Button>
          </DialogTrigger>
        ) : null}
        <DialogContent>
