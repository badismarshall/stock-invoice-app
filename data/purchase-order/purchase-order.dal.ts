    ...
    return {
      purchaseOrders: data.map((item) => {
        ...
        return {
          ...
          currency: item.purchaseOrder.currency || "DZD",
  ...
  try {
    ...
    return {
      ...
      supplier: item.supplier ? {
        ...
        nafApe: item.supplier.nafApe,
        rcsRm: item.supplier.rcsRm,
        eori: item.supplier.eori,
        tvaNumber: item.supplier.tvaNumber,
      } : null,
      supplierName: item.supplier?.name || null,
      orderDate,
      receptionDate,
      ...
      items: items.map((i) => ({
        ...
        taxRate: i.item.taxRate ? parseFloat(i.item.taxRate) : (i.product?.taxRate ? parseFloat(i.product.taxRate) : 0),
